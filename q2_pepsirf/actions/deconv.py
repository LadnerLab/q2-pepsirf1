...
def collect_cmd(
    ...
    cmd = "%s deconv -e %s -t %s -l %s --scoring_strategy %s --score_tie_threshold %s --score_overlap_threshold %s -o %s -s %s" % (
        pepsirf_binary, enriched, threshold, linked, scoring_strategy, score_tie_threhold, score_overlap_threshold, tsv_out, score_per_round
    )
...
def deconv_singular(
    ...
    with tempfile.TemporaryDirectory() as tempdir:
        ...
        cmd = collect_cmd(
            ...
            tsv_out, score_filtering, id_name_map, single_threaded,
