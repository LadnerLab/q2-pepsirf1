...
def demux(
    ...
    with tempfile.TemporaryDirectory() as tempdir:
        ...
        cmd = "%s demux --input_r1 %s --input_r2 %s --seq %s -r %s -t %s --phred_base %s -i %s -s %s --phred_min_sore %s --sname %s -d %s -o %s" % (
            pepsirf_binary, input_r1, input_r2, seq, read_per_loop, num_threads, 
            phred_base, index, samplelist, phred_min_score, sname, diagnostic_data, 
            raw_data
        )
